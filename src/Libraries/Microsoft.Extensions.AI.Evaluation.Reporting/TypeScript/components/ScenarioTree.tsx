...
export const ScenarioGroup = ({ node, renderMarkdown, selectedTags }: { 
  ...
  const filterTree = (node: ScoreNode): ScoreNode | null => {
    ...
    if (filteredChildren.length > 0) {
      ...
      // Re-aggregate the filtered node with the selected tags
...
export const ScoreDetail = ({ scenario, renderMarkdown }: { scenario: ScenarioRunResult, renderMarkdown: boolean }) => {
    ...
    const { messages, usage } = getConversationDisplay(scenario.messages, scenario.modelResponse);
    ...
    return (<div className={classes.iterationArea}>
        ...
        <ConversationDetails messages={messages} usage={usage} renderMarkdown={renderMarkdown} />
...
export const ConversationDetails = ({ messages, usage, renderMarkdown }: { 
    ...
    const usageText = usage ? 
        `${usage.inputTokenCount} input + ${usage.outputTokenCount} output = ${usage.totalTokenCount} total tokens` : 
        '';
    ...
    return (
        <div className={classes.section}>
            <div className={classes.sectionHeader} onClick={() => setIsExpanded(!isExpanded)}>
                ...
                {usage && <div className={classes.hint}>{usageText}</div>}
            </div>
...
export const ChatDetailsSection = ({ chatDetails }: { chatDetails: ChatDetails }) => {
    ...
    // Check if caching is enabled (any turn has cache key or cache hit info)
    const cachingEnabled = chatDetails.turnDetails.some(turn => 
        turn.cacheKey !== undefined || turn.cacheHit !== undefined);
    ...
    return (
        <div className={classes.section}>
            <div className={classes.sectionHeader} onClick={() => setIsExpanded(!isExpanded)}>
                {isExpanded ? <ChevronDown12Regular /> : <ChevronRight12Regular />}
                <h3 className={classes.sectionHeaderText}>LLM Diagnostic Details</h3>
                {cachingEnabled && (
                ...
                <div className={classes.sectionContainer}>
                    <div className={classes.tableContainer}>
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    {cachingEnabled && <TableHeaderCell>Cache Key</TableHeaderCell>}
                                    {cachingEnabled && <TableHeaderCell>Cache Status</TableHeaderCell>}
                            ...
                            <TableBody>
                                {chatDetails.turnDetails.map((turn, index) => (
                                    <TableRow key={index}>
                                        ...
                                        <TableCell>{turn.usage.inputTokenCount}</TableCell>
                                        <TableCell>{turn.usage.outputTokenCount}</TableCell>
                                        <TableCell>{turn.usage.totalTokenCount}</TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
