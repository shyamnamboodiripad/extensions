    ...
    IHttpClientFactory httpClientFactory)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        HttpResponseMessage response =
            await GetResponseAsync(serviceAvailabilityUrl, cancellationToken).ConfigureAwait(false);
    ...
    private async ValueTask<string> GetServiceDiscoveryUrlAsync(CancellationToken cancellationToken)
    {
        ...
        HttpResponseMessage response = await GetResponseAsync(requestUrl, cancellationToken).ConfigureAwait(false);
        if (!response.IsSuccessStatusCode)
        {
            throw new InvalidOperationException(
                $"""
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        string userAgent =
            string.IsNullOrWhiteSpace(evaluatorName)
                ? "azure-ai-evaluation/unknown"
                : $"azure-ai-evaluation/unknown (type=evaluator; subtype={evaluatorName})";
